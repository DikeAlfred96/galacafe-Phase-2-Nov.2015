		    ...
		    <div id="take_out">
			    ...
			    <div class="kitchen_process waterfall">
		    		...
		    		<div class="single_takeout waterfall-item">
		    			...
		    			<p>订单号: <?php echo $currentId; ?><span class="finish_order">完成<input type="hidden" value="<?php echo $currentId; ?>" class="order_id"></span></p>
		    	...
		    	<div class="admin_process waterfall">
			    	<h5>待取餐</h5>
			    	<?php foreach ($table_0_final->result() as $t_zero_final): ?>
		    			<div class="single_takeout waterfall-item">
		    			<?php $currentIdFinal = $t_zero_final->orderId; ?>
		    			<p>订单号: <?php echo $currentIdFinal; ?><span class="remove_order">确认取餐<input type="hidden" value="<?php echo $currentIdFinal; ?>" class="order_id"></span></p>
		    			<?php $sql_takeout_final = "SELECT dishId, serialId, dishQuantity, dishStatus, dishChiName, dishAlphaId FROM view_order_items WHERE orderId='{$currentIdFinal}';";
			    		$result_takeout_final = $this->db->query($sql_takeout_final); 
			    		foreach ($result_takeout_final->result() as $item_out_final): 
			    		$status = $item_out_final->dishStatus; ?>
		    				<p class="single_takeout_dish"><span class="dish_alpha"><?php echo $item_out_final->dishAlphaId; ?></span><span class="dish_name"><?php echo $item_out_final->dishChiName; ?></span><span class="dish_qty"><?php echo $item_out_final->dishQuantity; ?></span><input class="serial" type="hidden" value="<?php echo $item_out_final->serialId; ?>"></p>
		    			<?php endforeach; ?>
		    			</div>
		    		<?php endforeach; ?>
		    		</div>
