...
<div id="back_end_control_panel">
	...
	<!-- Tab panes -->
  	<div class="tab-content">
	    <div id="orderhistory">
			...
			<ul class="nav" role="tablist">
				<li role="presentation" class="active"><a href="#today" aria-controls="today" role="tab" data-toggle="tab">今天 | $ <strong id="total_amount_1">0.00</strong></a></li><li role="presentation"><a href="#yesterday" aria-controls="yesterday" role="tab" data-toggle="tab">昨天 | $ <strong id="total_amount_2">0.00</strong></a></li><li role="presentation"><a href="#older" aria-controls="older" role="tab" data-toggle="tab">更早</a></li>
			</ul>
		    	...
		    	<div role="tabpanel" class="tab-pane active" id="today">
			    	<table class="table-today">
					    ...
					    <?php foreach ($today_history->result() as $item):
								...
								$order_deliver_method = $item->orderDeliverMethod;
								$order_address = $item->orderAddress;
				    		...
				    		<tr class="<?php echo $odd; ?> order_single">
					    		...
					    		<input type="hidden" value="<?php echo $item->orderTotal; ?>" class="order_total_1 orderTotal_<?php echo $i; ?>">
					    		<input type="hidden" value="<?php echo $order_deliver_method; ?>" class="ship_method_<?php echo $i; ?>">
					    		<?php if ($order_deliver_method = 'Delivery') { ?><input type="hidden" class="address_<?php echo $i; ?>" value="<?php echo $order_address; ?>"><?php } ?>
			    	...
			    	<table class="table-yesterday">
					    ...
					    <?php foreach ($one_day_history->result() as $item):
				    		...
				    		<tr class="<?php echo $odd; ?> order_single">
					    		...
					    		<input type="hidden" value="<?php echo $order_subtotal; ?>" class="orderSubtotal_<?php echo $i; ?>">
					    		<input type="hidden" value="<?php echo $order_tax; ?>" class="orderTax_<?php echo $i; ?>">
					    		<input type="hidden" value="<?php echo $item->orderTotal; ?>" class="order_total_2 orderTotal_<?php echo $i; ?>">
					    		<input type="hidden" value="<?php echo $order_deliver_method; ?>" class="ship_method_<?php echo $i; ?>">
					    		<?php if ($order_deliver_method = 'Delivery') { ?><input type="hidden" class="address_<?php echo $i; ?>" value="<?php echo $order_address; ?>"><?php } ?>
			    	...
			    	<table class="table-older">
					    ...
					    <?php foreach ($older_history->result() as $item):
				    		...
				    		<tr class="<?php echo $odd; ?> order_single">
					    		...
					    		<input type="hidden" value="<?php echo $order_subtotal; ?>" class="orderSubtotal_<?php echo $i; ?>">
					    		<input type="hidden" value="<?php echo $order_tax; ?>" class="orderTax_<?php echo $i; ?>">
					    		<input type="hidden" value="<?php echo $item->orderTotal; ?>" class="order_total_3 orderTotal_<?php echo $i; ?>">
					    		<input type="hidden" value="<?php echo $order_deliver_method; ?>" class="ship_method_<?php echo $i; ?>">
					    		<?php if ($order_deliver_method = 'Delivery') { ?><input type="hidden" class="address_<?php echo $i; ?>" value="<?php echo $order_address; ?>"><?php } ?>
  	...
  	<script>
  		$("input.order_total_1").sum("keyup", "#total_amount_1");
  		$("input.order_total_2").sum("keyup", "#total_amount_2");
	  	...
	  	function printDiv(index, section) {
		  	...
		  	var shipMethod = document.getElementById(section).getElementsByClassName('ship_method_'+index)[0].value;
		  	if (shipMethod == 'Pick') {
		  		var method = '自取';
		  	} else {
		  		var method = '送餐';
		  	...
		  	if (tableId == 0) {
			  	...
			  	tableId = '<span class="table_id">外卖-' + method + '</span>' + aliasId;
			} else {
				tableId = tableId;
			}
		  	...
		  	var orderTotal = document.getElementById(section).getElementsByClassName('orderTotal_'+index)[1].innerHTML;
		    ...
		    if (document.getElementById(section).getElementsByClassName('userTel_'+index)[0] != null) {
		  	...
		  	if (shipMethod == 'Delivery') {
		  		var address = document.getElementById(section).getElementsByClassName('address_'+index)[0].value;
